...
// jette des exceptions lorsque du code non strict (par exemple, ASP.NET 4.5) accède au mode strict
...
var isWindow = la fonction isWindow (obj) {
	...
	fonction DOMEval (code, doc, noeud) {
		...
		script.text = code;
		si (noeud) {
			pour (i en ConservScriptAttributes) {
				si (noeud [i]) {
					script [i] = noeud [i];
				}
			}
...
/ * Symbole global * /
// Définir ce global dans .eslintrc.json risquerait d’utiliser le global
// sans surveillance à un autre endroit, il semble plus sûr de définir uniquement global pour ce module
...
jQuery.extend ({
	...
	isPlainObject: fonction (obj) {
		...
		// Utiliser toString au lieu de jQuery.type pour intercepter des objets hôtes
		...
		type de retour de Ctor === "function" && fnToString.call (Ctor) === ObjectFunctionString;
	},

	isEmptyObject: function (obj) {

		/ * eslint-désactiver les vars-inutilisés * /
	...
	// Evalue un script dans un contexte global
	...
	// push.apply (_, arraylike) lance l'ancien WebKit
...
var i,
	...
	// sérialisation des chaînes / identifiants CSS
	// https://drafts.csswg.org/cssom/#common-serializing-idioms
	rcssescape = / ([\ 0- \ x1f \ x7f] | ^ -? \ d) | ^ - $ | [^ \ 0- \ x1f \ x7f- \ uFFFF \ w -] / g,
	fcssescape = function (ch, asCodePoint) {
		if (asCodePoint) {

			// U + 0000 NULL devient le caractère de remplacement U + FFFD
			if (ch === "\ 0") {
				renvoyer "\ uFFFD";
			}
...
function createDisabledPseudo (désactivé) {
	...
	// connu: désactivé faux positifs: champset [désactivé]> légende: nth-de-type (n + 2): peut-désactiver
	fonction de retour (elem) {
		...
		si ("forme" dans elem) {
			...
			// Rechercher les handicaps hérités sur les éléments non désactivés pertinents:
			// * liste des éléments associés aux formulaires dans un jeu de champs désactivé
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				// Les éléments optionnels sont transférés à un optgroup parent s'il est présent
				if ("label" dans elem) {
				...
				// Support: IE 6 - 11
				// Utilise la propriété de raccourci isDisabled pour rechercher les ancêtres de jeu de champs désactivés
				return elem.isDisabled === désactivé ||

					// S'il n'y a pas de isDisabled, vérifiez manuellement
					/ * jshint -W018 * /
		...
		// Essayez de vider les éléments qui ne peuvent pas être désactivés avant de faire confiance à la propriété désactivée.
		// Certaines victimes se font prendre dans notre réseau (étiquette, légende, menu, piste), mais cela ne devrait pas
		// existe même sur eux, et encore moins une valeur booléenne.
...
setDocument = Sizzle.setDocument = fonction (noeud) {
	...
	// Les méthodes getElementById cassées ne récupèrent pas les noms définis par programmation,
		...
		Expr.find ["ID"] = fonction (id, contexte) {
			if (typeof context.getElementById! == "undefined" && documentIsHTML) {
				...
				si (elem) {
					...
					// Retourner sur getElementsByName
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		affirmer (fonction (el) {
			...
			// In-page `selector # id sibling-combinator selector` échoue
		...
		affirmer (fonction (el) {
			...
			// Support: IE9-11 +
			// IE: le sélecteur désactivé ne capte pas les enfants des jeux de champs désactivés
...
Sizzle.escape = function (sel) {
	retourne (sel + "") .replace (rcssescape, fcssescape);
};
